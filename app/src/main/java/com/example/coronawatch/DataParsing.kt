...
import com.example.coronawatch.classes.CountryInfo
...
import com.example.coronawatch.ui.socialMedia.Robots
...
fun robotsParsing(dataString: String, nContext: Context?): RobotsFeed {

    val data = JSONObject(dataString)
    val json = data.getJSONArray("results")
    var feed = arrayListOf<Robots>()
    ...
    while (i >= 0) {
        ...
        val jsonRobot = json.getJSONObject(i)


        feed.add(
            Robots(

                jsonRobot["titre"].toString(),
                jsonRobot["langue"].toString(),
                jsonRobot["source"].toString(),
                jsonRobot["lien"].toString()
    ...
    return RobotsFeed(feed , data.getString("next") , data.getString("next") != "null")
}
